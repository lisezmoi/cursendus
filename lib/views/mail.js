...
SmtpView.prototype.start = function(gamesManager, template, pubsub, errorMessages, logger, conf) {
  ...
  function handleMessage(message) {
    ...
    logger.info('Incoming message: %d email addresse(s)', addresses.length);

    if (addresses.length === 0 || !message.from[0] || !message.from[0].address) {
      // Error
      logger.info('Error: missing the `from` address.');
      return;
    }
        ...
        if (player !== false) {
          ...
          if (commands.length > 0) {
            ...
            if (oneValidCommand) { // At least one valid command
              ...
              logger.info('Invalid commands in the message.');
            ...
            logger.info('Missing commands in the message.');
            messageBody(errorMessages['command'], conf.assetsUrl, template, function(body) {
              sendMessage(fromAddress, game.id, true, body);
            });
            return;
          ...
          logger.info('The user is not a player of this game');
        }
      });
    ...
    // Not enough emails to create a game
      ...
      logger.info('Game not created: 2 mails are needed to create a game.');
      return;
    }
