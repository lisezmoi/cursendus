...
function getHtmlBody(game, activePlayer, assetsUrl, template, cb) {
...
SmtpView.prototype.start = function(gamesManager, template, pubsub, errorMessages, conf) {
  var self = this,
      transport;
  ...
  function handleMessage(message) {
          ...
          if (commands.length > 0) {
            ...
            if (lastCommandStatus) {
              ...
              if (game.commandsEnd(player)) {
                getHtmlBody(game, game.player1, conf.assetsUrl, template, function(body) {
                  sendMessage(game.player1.email, game, body);
                });
                getHtmlBody(game, game.player2, conf.assetsUrl, template, function(body) {
                  sendMessage(game.player2.email, game, body);
                });
              };
            } else {
    ...
    // The game does not exists, try to create a new game.
      ...
      gamesManager.addGame(addresses[0], addresses[1], function(game) {
        getHtmlBody(game, game.player1, conf.assetsUrl, template, function(body) {
          sendMessage(addresses[0], game, body);
        });
        getHtmlBody(game, game.player2, conf.assetsUrl, template, function(body) {
          sendMessage(addresses[1], game, body);
        });
      });
