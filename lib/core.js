    ...
    for (var j=0; j < terrain[i].length; j++) {
      if (i < 4) {
        ...
        if (!moon) {
          ...
          if (terrain[i][j].skin === skinTiles.sky[skinTiles.sky.length-1]) {
            moon = true;
          }
        } else {
          terrain[i][j].skin = skinTiles.sky[getRandomInt(0, skinTiles.sky.length-2)];
        }
      } else if (i === 4) {
...
Core.prototype.command = function(gameId, playerId, command) {
